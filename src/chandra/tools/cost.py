...
def find_xray_high_error_rate(ctx: DetectorContext) -> list[Finding]:
    ...
    Calls ``get_service_graph`` for a rolling 1-hour window (the maximum
    resolution X-Ray retains at this granularity) and inspects the
    ...
    to the combined rate, because either degrades end-user experience.

    Services with fewer than 10 total requests are skipped — low traffic makes
    rates statistically unreliable.
    """
    detector_id = "PERF-004-xray-error-rate"
    findings: list[Finding] = []
    now = datetime.now(timezone.utc)
    # X-Ray service graph requires a window; we use the last hour.
    ...
    for region in ctx.regions:
        ...
        for node in service_nodes:
            # Skip infrastructure edges (e.g. downstream AWS services that are
            # not the root cause) — only evaluate REMOTE and local service types.
            ...
            # X-Ray doesn't expose a proper ARN for service nodes; synthesise one.
            arn = (
...
def find_xray_high_latency(ctx: DetectorContext) -> list[Finding]:
    ...
    (e.g. very low traffic nodes), the detector falls back to
    ``SummaryStatistics.ResponseTime`` (average) as a conservative proxy.
    ...
    for region in ctx.regions:
        ...
        for node in service_nodes:
            ...
            # Compute p99 from the histogram when available.
            histogram = node.get("ResponseTimeHistogram", [])
            p99_ms: float | None = None
            if histogram:
                # Histogram entries: {"Value": seconds, "Count": n}
                # Sort ascending and walk until we reach the 99th-percentile bucket.
            ...
            if p99_ms is None:
                # Fall back to average response time as a conservative proxy.
            ...
            findings.append(
                Finding(
                    ...
                    recommendation=(
                        ...
                        "Look for slow database queries (RDS/DynamoDB segments), "
                        "synchronous downstream calls, or cold-start latency on Lambda. "
                        "Consider adding CloudWatch Contributor Insights rules to "
                        "surface the slowest individual operations."
                    ),
                    detector_id=detector_id,
                )
            )
...
def find_compute_optimizer_ec2(ctx: DetectorContext) -> list[Finding]:
    ...
    Calls ``get_ec2_instance_recommendations`` and emits one finding per
    instance whose ``Finding`` field is ``OVER_PROVISIONED``.  The top-ranked
    ``RecommendationOptions`` entry (index 0, lowest estimated cost) is
    included in the evidence so engineers can act immediately without opening
    the console.

    Requires that Compute Optimizer is enabled in the account (opt-in).  If
    it is not enabled the API raises ``OptInRequiredException``; the
    ``detector_guard`` swallows this gracefully and logs a warning.
    """
    ...
    for region in ctx.regions:
        ...
        with detector_guard(ctx, detector_id=detector_id, region=region):
            for page in paginate(co, "get_ec2_instance_recommendations"):
                recommendations.extend(
                    page.get("instanceRecommendations", [])
                )
        ...
        for rec in recommendations:
            ...
            # Best option: index 0 is Compute Optimizer's top recommendation.
            ...
            # Estimated monthly savings from Compute Optimizer (may be absent).
            ...
            severity = "medium"
            if monthly_savings is not None and monthly_savings >= 100:
                severity = "high"
            ...
            if monthly_savings is not None:
                title_parts.append(
                    f" (est. ${monthly_savings:.2f}/mo savings)"
                )
...
def find_compute_optimizer_lambda(ctx: DetectorContext) -> list[Finding]:
    """Surface Compute Optimizer Lambda recommendations where finding is OVER_PROVISIONED.

    Evaluates memory configuration recommendations from Compute Optimizer.
    An over-provisioned Lambda function is billed for memory it does not use;
    reducing it also reduces per-GB-second costs proportionally.

    As with PERF-006, the top recommendation option is surfaced in evidence
    and the severity escalates to ``high`` when estimated monthly savings
    exceed $100.
    """
    ...
    for region in ctx.regions:
        ...
        recommendations: list[dict[str, Any]] = []
        with detector_guard(ctx, detector_id=detector_id, region=region):
            for page in paginate(co, "get_lambda_function_recommendations"):
                recommendations.extend(
                    page.get("lambdaFunctionRecommendations", [])
                )
            ...
            findings.append(
                Finding(
                    ...
                    recommendation=(
                        ...
                        f"{current_memory} MB to {recommended_memory} MB. Run a "
                        "round of integration tests to confirm p99 duration stays "
                        "within acceptable bounds — lower memory reduces CPU share "
                        "and may slightly increase execution time. Use AWS Lambda "
                        "Power Tuning (open-source Step Functions tool) to validate "
                        "the optimal memory/cost trade-off for your workload."
                    ),
                    detector_id=detector_id,
                )
            )
