...
Flow:
    1. GeneratorAgent creates files in a sandbox (no feedback on first run).
    2. ExecutorAgent runs those files.
    3. If execution succeeds  → return final result.
    4. If execution fails     → pass executor's summary as feedback_summary
                                back to GeneratorAgent, which reads the
                                existing files and patches them.
...
logging.basicConfig(
    filename="orchestrator.log",  # The file where logs will be saved
    filemode="w",                 # "a" appends to the file, "w" overwrites it on each run
    level=logging.INFO,
...
class OrchestratorResponse(BaseModel):
    ...
    questions: Optional[List[str]] = None  # populated if generator needs HITL
...
class OrchestratorAgent:
    ...
    def RunPipeline(
        ...
        Args:
            action:              Dict with actionName, actionDescription, steps.
            sandbox_path:        Optional pre-existing sandbox to resume from.
            thread_id:           Orchestrator-level thread ID (auto-generated if None).
            answers:             Answers to HITL questions from a previous paused run.
            generator_thread_id: The generator's thread_id when resuming a HITL pause.
            command_timeout:     Per-command timeout in seconds (default: 300 = 5 minutes).
                                 If a command (e.g. terraform plan) exceeds this limit it is
                                 killed, its timeout is reported in the summary, and the
                                 generator receives that summary as feedback to fix the issue.
        """
        orch_tid = thread_id or str(uuid.uuid4())
        records: List[IterationRecord] = []
        feedback: Optional[str] = None
        current_sandbox: Optional[str] = sandbox_path
        gen_tid: Optional[str] = generator_thread_id  # carry across HITL resume
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            if answers and gen_tid:
                ...
                answers = None          # consume answers; won't re-use
                gen_tid = None
            else:
            ...
            # HITL — generator needs clarification from user
            if gen_response.status == "needs_clarification":
                ...
                return OrchestratorResponse(
                    ...
                    # Caller must pass generator_thread_id when resuming
                    summary=(
            ...
            logger.info(
                "[Iteration %d] GeneratorAgent succeeded. sandbox=%s files=%d",
                iteration,
                current_sandbox,
                len(gen_response.executableSteps or []),
            )
            ...
            # Execution failed — build feedback for next generator call
            logger.warning(
            ...
            logger.info("[Iteration %d] Feedback summary: %s", iteration, feedback)
        ...
        last_exec = exec_response  # the final failed attempt
        return OrchestratorResponse(
            statusCode=207,
            status="failed",
            thread_id=orch_tid,
...
if __name__ == "__main__":
    ...
    response = orchestrator.RunPipeline(
        ...
        command_timeout=1600,   # 5 minutes per command; increase for slow apply steps
    )
    print(response.model_dump_json(indent=2))

    # ── HITL resume example (uncomment if needed) ──────────────────
    # if response.status == "needs_clarification":
    #     response = orchestrator.RunPipeline(
    #         action=action_payload,
    #         thread_id=response.thread_id,
    #         answers=["vpc-0123456789abcdef0", "subnet-0123456789abcdef0"],
    #         generator_thread_id="<generator_thread_id from response.summary>",
    #     )
    #     print(response.model_dump_json(indent=2))