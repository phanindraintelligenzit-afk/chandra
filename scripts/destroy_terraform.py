...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Automatically approve the destruction (default: True)",
    )
    parser.add_argument(
        "--no-auto-approve",
        action="store_false",
        dest="auto_approve",
        help="Prompt for approval before destroying",
    )
    parser.add_argument(
        "--jiraUrl", help="Jira URL to update after successful destruction", default=None
    )
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            if args.jiraUrl:
                try:
                    ...
                    from tools.jira_tools.create_jira_ticket import (
                        add_label_to_ticket,
                        add_summary_comment,
                    ...
                    for status in statuses_to_try:
                        logger.info(
                            f"Attempting to transition Jira ticket {issue_key} to '{status}'"
                        )
                    ...
                    if not transitioned:
                        logger.warning(
                            f"Could not transition {issue_key} to any of {statuses_to_try}. Check your Jira workflow."
                        )
                except Exception as e:
        ...
        logger.error(
            "'terraform' command not found. Please ensure Terraform is installed and in your PATH."
        )
        sys.exit(1)
    except Exception as e:
