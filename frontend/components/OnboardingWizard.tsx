...
import { CheckCircle2, KeyRound, ListChecks, Search, ShieldCheck, Sparkles, X } from "lucide-react";
...
import { AwsTasksOnboardingStep } from "./AwsTasksOnboardingStep";
import { AwsPermissionsOnboardingStep } from "./AwsPermissionsOnboardingStep";
...
export default function OnboardingWizard() {
  ...
  const canNext = useMemo(() => {
    ...
    if (step === 4) return true;  // AWS Tasks - always can proceed
    if (step === 5) return true;  // AWS Permissions - always can proceed
    if (step === 6) return false; // Deployment - auto-advances
    return true;
  ...
  function next() {
    ...
    if (step === 3) {
      ...
      // KRA step → mark KRAs as complete, then go to AWS Tasks
      setStep(4);
      return;
    }
    if (step === 4) {
      // AWS Tasks → AWS Permissions
      completeStep("aws-tasks");
      setStep(5);
      return;
    }
    if (step === 5) {
      // AWS Permissions → Deployment
      completeStep("aws-permissions");
      completeStep("aws-tasks");
      setStep(6);
  ...
  async function runDeploymentSequence() {
    ...
    try {
      ...
      if (!controller.signal.aborted) {
        ...
        // Fire cost metrics fetch in background
        ...
        // Fire observations fetch in background
        const obsController = new AbortController();
        const obsTimeout = setTimeout(() => obsController.abort(), 600_000);
  ...
  // Steps 4 (AWS Tasks) and 5 (AWS Permissions) are handled in separate components.
  // Step 6 (Deployment) is the deployment progress screen.
  // After deployment completes, routes to Dashboard.
  const SHOW_STANDARD_STEP = step <= 3 || step === 6;
      ...
      <div className="onboarding-shell relative z-10 w-full max-w-5xl rounded-2xl border border-signal/25 bg-gradient-to-b from-black/70 via-black/50 to-black/40 p-6 shadow-[0_30px_80px_rgba(255,59,59,0.18),0_0_0_1px_rgba(255,59,59,0.08)] backdrop-blur">
        ...
        {SHOW_STANDARD_STEP ? (
          <AnimatePresence mode="wait">
            {/* Step 0: Worker Details (Name + Avatar + Gender) */}
            {step === 0 && (
            ...
            {/* Step 1: AWS Cloud Engineer (Role) */}
            {step === 1 && (
            ...
            {/* Step 3: Monitoring/KRA (KRA selection) */}
            {step === 3 && (
              <motion.div key="kras" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                      ...
                      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                        <div className="flex flex-wrap items-center gap-3">
                          <label className="flex cursor-pointer items-center gap-2 rounded-full border border-white/10 bg-black/40 px-3 py-1.5 transition hover:border-emerald-300/30">
                            ...
                            />
                            <span className="text-[0.58rem] uppercase tracking-[0.18em] text-frost/80">{`${selectedCount}/${customKras.length} Selected`}</span>
                          </label>
                          <button
                            type="button"
                        ...
                        <div className="relative w-full sm:w-72">
                          ...
                          />
                          {customKraSearch ? (
                            <button type="button" onClick={() => setCustomKraSearch("")} aria-label="Clear search" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full p-1 text-muted transition hover:bg-white/10 hover:text-frost">
                              <X size={12} />
                            </button>
                          ) : null}
                        </div>
                ...
                <div className="mt-6 flex items-center gap-3">
                  <button onClick={prev} className="rounded-2xl border border-white/10 px-4 py-3 text-sm uppercase tracking-[0.14em] text-muted">BACK</button>
                  <button onClick={next} className="ml-auto rounded-2xl bg-emerald-300/10 px-5 py-3 text-sm font-semibold uppercase tracking-[0.14em] text-emerald-200">CONTINUE TO AWS TASKS</button>
                </div>
              </motion.div>
            )}

            {/* Step 6: Deployment (progress screen) */}
            {step === 6 && (
              <motion.div key="deploy" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="mx-auto max-w-xl text-center">
                  ...
                  <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-signal/30 bg-black/40 px-3 py-1 text-[0.6rem] uppercase tracking-[0.18em] text-signal">
                    ...
                    {observationsStatus === "loading" ? "RETRIEVING LIVE OPERATIONAL INTELLIGENCE..." : observationsStatus === "success" ? "LIVE OPERATIONAL INTELLIGENCE RECEIVED" : observationsStatus === "error" ? "LIVE DEPLOYMENT SYNC PAUSED" : "AWAITING BACKEND SYNC"}
                  </div>
                  {observationsStatus === "error" && observationsErrorMessage ? (
                    <div className="mt-3"><p className="text-[0.7rem] text-amber/80">{observationsErrorMessage}</p></div>
                  ) : null}
          ...
          /* Step 4: AWS Tasks (full CRUD) */
          <AwsTasksOnboardingStep
            onNext={next}
            onBack={prev}
            agentName={agentName || normalizedName || "Agent"}
          />
        ) : step === 5 ? (
          /* Step 5: AWS Permissions (full CRUD) */
          <AwsPermissionsOnboardingStep
            onNext={next}
            onBack={prev}
            agentName={agentName || normalizedName || "Agent"}
          />
