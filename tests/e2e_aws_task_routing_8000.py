    ...
    for i in range(max_polls):
        ...
        try:
            ...
            if status == "completed" and "Awaiting user input" in str(msg):
                print(
                    f"[{i + 1}/{max_polls}] HITL pause detected. Resuming job with answer 'yes'..."
                )
                res_resume = requests.post(
                    f"{BASE_URL}/orchestrate/{actual_job_id}/resume", json={"answers": ["yes"]}
                )
            ...
            if status in [
                ...
                "blocked",
                "unverified",
            ] and "Awaiting user input" not in str(msg):
