...
from tests.test_nodes._env import aws_scope, banner, make_state, mode_banner, real_region, real_account_id


def _write(
    detector_id: str, target_arn: str, region: str | None = None
) -> ProposedWrite:
...
def test_actionexecutor() -> dict:
    ...
    with aws_scope():
        try:
            ...
            if code in ("AccessDenied", "BucketAlreadyExists", "BucketAlreadyOwnedByYou"):
                ...
                print(f"    - Warning: Could not create test bucket {bucket} ({code}), ignoring... Details: {msg}")
            else:
                raise
    ...
    for r in result["action_results"]:
        print(
            f"    - action={r.action:30s}  status={r.status:8s}"
            f"  dry_run={r.dry_run}"
        )
        if r.audit_log:
