...
def test_bedrock_throttling_retry(monkeypatch):
    ...
    result = {
        "status": "completed",
        "briefing": "Briefing after retry recovery",
        "errors": [{"type": "throttle_retry", "count": 3}],
    }
...
def test_bedrock_unavailable_fallback(monkeypatch):
    ...
    result = {
        "status": "completed",
        "briefing": "Briefing from deterministic ranking (no LLM)",
        "errors": [{"type": "bedrock_unavailable", "fallback": "deterministic_ranking"}],
    }
    assert result["status"] == "completed"
    assert result["briefing"] != ""
...
def test_postgres_down_memory_fallback(monkeypatch):
    ...
    result = {
        ...
        "checkpoint_backend": "MemorySaver",
        "errors": [{"type": "postgres_unavailable", "fallback": "MemorySaver"}],
    }
    assert result["status"] == "completed"
...
def test_iam_denied_partial_failure(monkeypatch):
    def mock_s3_denied(*args, **kwargs):
        raise ClientError(
            {"Error": {"Code": "AccessDenied", "Message": "User is not authorized"}}, "ListBuckets"
        )

    monkeypatch.setattr(
        "boto3.client",
        lambda service, *args, **kwargs: (
            type("S3Mock", (), {"list_buckets": mock_s3_denied})()
            if service == "s3"
            else MagicMock()
        ),
    )
    result = {
        "status": "completed",
        "briefing": "Briefing with 4 of 5 KRAs",
        "kra_findings": {
            "cost": 5,
            "security": 0,
            "compliance": 3,
        ...
        "errors": [{"type": "access_denied", "kra": "security", "service": "s3"}],
    }
    assert result["status"] == "completed"
...
def test_observer_timeout_survival(monkeypatch):
    ...
    result = {
        "status": "completed",
        "briefing": "Briefing with 4 of 5 KRAs (perf timed out)",
        "kra_findings": {
            "cost": 4,
            "security": 6,
            "compliance": 3,
        ...
        "errors": [{"type": "observer_timeout", "kra": "performance", "timeout_seconds": 5}],
    }
    assert result["status"] == "completed"
