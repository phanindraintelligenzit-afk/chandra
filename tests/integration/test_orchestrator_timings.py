...
def test_execute_node_timing_initialization(mock_execution_agent):
    ...
    """
    state = {
        "action": {"actionName": "Test AWS Task", "service": "s3", "action_type": "AWS_TASK"},
        "plan_review": {"commands": [{"command": "echo 'terraform apply'", "order": 1}]},
        "sandbox_path": "terraform_runs/test_worker/test-timings-1234",
        "command_timeout": 10,
        "pre_apply_results": [],
        "aws_permissions": [{"id": "S3FullAccess", "actions": [{"action": "s3:*"}]}],
        "execution_plan": {"commands": [{"command": "echo 'terraform apply'", "order": 1}]},
    }
    ...
    with patch.object(mock_execution_agent, "_run_commands") as mock_run:
        ...
        with patch(
            "digitalworker_agents.aws_execution_agent.execute_shell_command"
        ) as mock_exec_shell:
