...
class TestPublicS3:
    ...
    def test_bucket_with_full_block_is_not_flagged(self, ctx: DetectorContext, s3: Any) -> None:
...
class TestOpenSecurityGroups:
    def _make_sg_with_ingress(self, ec2: Any, port: int, cidr: str = "0.0.0.0/0") -> str:
        vpcs = ec2.describe_vpcs()["Vpcs"]
        vpc_id = (
            vpcs[0]["VpcId"] if vpcs else ec2.create_vpc(CidrBlock="10.0.0.0/16")["Vpc"]["VpcId"]
        )
        sg = ec2.create_security_group(GroupName=f"open-{port}", Description="test", VpcId=vpc_id)
        sg_id = sg["GroupId"]
    ...
    def test_internal_cidr_is_not_flagged(self, ctx: DetectorContext, ec2: Any) -> None:
...
class TestStaleAccessKeys:
    ...
    def test_recent_key_is_not_flagged(self, ctx: DetectorContext, iam: Any) -> None:
...
class TestWildcardIam:
    def test_wildcard_policy_is_critical(self, ctx: DetectorContext, iam: Any) -> None:
        doc = {
            "Version": "2012-10-17",
            "Statement": [
    ...
    def test_scoped_policy_is_not_flagged(self, ctx: DetectorContext, iam: Any) -> None:
        doc = {
            "Version": "2012-10-17",
            "Statement": [
                {
