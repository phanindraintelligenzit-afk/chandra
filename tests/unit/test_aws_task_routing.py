...
def test_kra_detector_still_routes_correctly(mock_action_executor, mock_execution_agents):
    ...
    mock_action_executor.return_value = {
        "action_results": [{"status": "success", "result": "done"}]
    }
...
def test_aws_task_status_skipped_never_becomes_completed(
    ...
    mock_action_executor.return_value = {
        "action_results": [{"status": "skipped", "result": "Not applicable"}]
    }
...
def test_duplicate_approval_prevented():
    ...
    response = client.post(
        "/requests/job_duplicate/approve", json={"approved": True, "approver": "test"}
    )
...
def test_aws_execution_agent_generate_node_syntax():
    ...
    agent.Llm = mock.Mock()
    agent.Llm.invoke.return_value = mock.Mock(
