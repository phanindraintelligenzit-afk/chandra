...
def test_cost_003_refuses_associated_eip(ec2: object) -> None:
    ...
    r = result["action_results"][0]
    # State-changed races surface as status="skipped" (the _SkippedRemediation
    # sentinel: not an error, just no longer applicable) — never as failure,
    # and critically never as a release of an in-use address.
